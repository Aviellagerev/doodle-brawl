...
export default function DrawingBoard({ isDrawer, socket }: Props) {
    ...
    function handlePointerMove(e: React.PointerEvent<HTMLCanvasElement>) {
        ...
        drawSegment(ctx, lastRef.current, p, color, segWidth, erasing);
    ...
    // receive + replay remote strokes and clears
    useEffect(() => {
        if (!socket) return;
        function onRemoteDraw(seg: DrawSegment) {
            const canvas = canvasRef.current;
            const ctx = canvas?.getContext("2d");
            ...
            {isDrawer && (
                <div className="bg-card flex items-center flex-wrap gap-x-3 gap-y-2" style={{ borderRadius: 16, boxShadow: "0 8px 20px rgba(58,47,38,.12)", padding: "12px 14px" }}>
                    ...
                    <button
                        ...
                        className="font-bold cursor-pointer text-ink"
                        style={{ border: "2.5px solid var(--outline)", borderRadius: 11, background: erasing ? "var(--amber)" : "var(--card)", padding: "7px 11px", fontSize: 12, boxShadow: erasing ? "2.5px 2.5px 0 var(--outline)" : "none" }}
                    >
                        Eraser
                    </button>
                    ...
                    <button
                        onClick={handleClear}
                        className="font-bold cursor-pointer text-card bg-rose ml-auto"
